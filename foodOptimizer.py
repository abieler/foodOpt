...
class Window(QTG.QMainWindow, ui_foodOptimizer.Ui_MainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.language = 'D'
        self.connect(self.pushButton_search,
            QTC.SIGNAL("clicked()"), self.search)
        self.connect(self.pushButton_startOptimization,
            QTC.SIGNAL("clicked()"), self.startOptimization)
        self.connect(self.tableWidget_overview,
            QTC.SIGNAL("cellClicked(int, int)"), self.addItemClicked)
        self.connect(self.tableWidget_userSelection,
            QTC.SIGNAL("cellClicked(int, int)"), self.removeItemClicked)
        self.connect(self.tableWidget_userSelection,
            QTC.SIGNAL("cellChanged(int, int)"), self.modifyCellClicked)
        self.connect(self.lineEdit_search,
            QTC.SIGNAL("textChanged(const QString&)"), self.search)

        self.message1.connect(self.updateProgressBar, QTC.Qt.QueuedConnection)
        self.add_menu_options()

        self.connect(self.tableWidget_overview.horizontalHeader(),
            QTC.SIGNAL("sectionResized(int, int, int)"),
            self.tableWidget_overview, QTC.SLOT("resizeRowsToContents()"))

        self.x0 = False
        self.optimizer_thread = Optimizer()
        self.optimizer_thread.message.connect(self.plotOptimizedResult,
        ...
        self.cur.execute("SELECT NAME_%s FROM naerwerte ORDER BY NAME_%s" %
        ...
        self.tableWidget_userSelection.setHorizontalHeaderLabels(
            ['Name', 'min [g]', 'max [g]', 'optimal', '  '])

        self.tableWidget_userSelection.setColumnWidth(0,
    ...
    def updateViewGraph(self):
        ...
        self.mpl_widget.ax.plot(np.arange(-1, len(indices) + 1),
            np.ones(len(indices) + 2) * 100, '--k', linewidth=2)
        self.mpl_widget.ax.plot(np.arange(-1, len(indices) + 1),
            np.zeros(len(indices) + 2), '-k')

        try:
            self.mpl_widget.ax.bar(indices - width1 / 2, finalResultMax, width1,
                facecolor='#32A0DE', label='max')
        except Exception:
            pass

        try:
            self.mpl_widget.ax.bar(indices - width2 / 2, finalResultMin, width2,
                facecolor='#1C2F39', label='min')
        except Exception:
            pass
    ...
    def plotOptimizedResult(self,msg):
        ...
        for k in range(nrOfRows):
            ...
            fat_i, protein_i, energy_kca_i, charbohydr_i, charbohydr2_i, \
            sugar_i, sodium_i, vit_a_i, b1_i, b2_i, b6_i, b12_i = \
            self.getFoodInformation(itemName, self.cur)
    ...
    def addItemClicked(self, i, j, loaded=False, foodItem=None, userMin=None,
        userMax=None, userOpt=None):
